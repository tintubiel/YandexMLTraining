...
class KNearestNeighbor:
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                sum_train = np.sum(self.X_train[j] ** 2)
                sum_test = np.sum(X[i] ** 2)
                dists[i][j] = np.sqrt(np.add.outer(sum_train, sum_test) - 2 * np.dot(self.X_train[j], X[i].T))
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            sum_test = np.sum(X[i] ** 2)
            dists[i] = np.sqrt(np.add.outer(sum_train, sum_test) - 2 * np.dot(self.X_train, X[i].T))
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.sqrt(np.add.outer(sum_train, sum_test) - 2 * np.dot(self.X_train, X.T))
        return dists.T
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
